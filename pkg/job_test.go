...
func TestAuthorize_Success(t *testing.T) {
	...
	terraformWorkspace := new(interfaces.TerraformWorkspaceMock)
	terraformerExecutor := new(interfaces.TerraformerExecutorMock)
	terraformImportMigrationGenerator := new(interfaces.TerraformImportMigrationGeneratorMock)
	resourcesCalculator := new(interfaces.ResourcesCalculatorMock)
	resourcesWriter := new(interfaces.ResourcesWriterMock)
	dragonDrop := new(interfaces.DragonDropMock)
	identifyCloudActors := new(interfaces.IdentifyCloudActorsMock)
	costEstimator := new(interfaces.CostEstimationMock)
...
type JobDependenciesMock struct {
	vcs                               *interfaces.VCSMock
	terraformWorkspace                *interfaces.TerraformWorkspaceMock
	terraformerExecutor               *interfaces.TerraformerExecutorMock
	terraformImportMigrationGenerator *interfaces.TerraformImportMigrationGeneratorMock
	resourcesCalculator               *interfaces.ResourcesCalculatorMock
	resourcesWriter                   *interfaces.ResourcesWriterMock
	dragonDrop                        *interfaces.DragonDropMock
	identifyCloudActors               *interfaces.IdentifyCloudActorsMock
	costEstimator                     *interfaces.CostEstimationMock
	driftDetector                     *interfaces.TerraformManagedResourcesDriftDetectorMock
	terraformSecurity                 *interfaces.TerraformSecurityMock
}
...
func createValidJob(t *testing.T) (*JobDependenciesMock, *Job) {
	...
	identifyCloudActors := new(interfaces.IdentifyCloudActorsMock)
	costEstimator := new(interfaces.CostEstimationMock)
	driftDetector := new(interfaces.TerraformManagedResourcesDriftDetectorMock)
	tfSec := new(interfaces.TerraformSecurityMock)
