...
func (c *HTTPDragonDropClient) getResourceInventoryData(ctx context.Context) (ResourceInventoryData, map[string]interface{}, error) {
	...
	deletedResources, err := readOutputFileAsSlice("drift-resources-deleted.json")
	if err != nil {
		return ResourceInventoryData{}, map[string]interface{}{}, fmt.Errorf("[error getting resources outside terraform control]%w", err)
	}

	return ResourceInventoryData{
		DriftedResources:                 len(driftedResources) + len(deletedResources),
