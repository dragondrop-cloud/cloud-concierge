...
func (f *Factory) Instantiate(ctx context.Context, environment string, vcs interfaces.VCS, provider terraformValueObjects.Provider, hclConfig hclcreate.Config) (interfaces.ResourcesWriter, error) {
	switch environment {
	case "isolated":
		return new(IsolatedResourcesWriter), nil
	default:
		return f.bootstrappedResourceWriter(ctx, vcs, provider, hclConfig)
	}
}
...
func (f *Factory) bootstrappedResourceWriter(ctx context.Context, vcs interfaces.VCS, provider terraformValueObjects.Provider, hclConfig hclcreate.Config) (interfaces.ResourcesWriter, error) {
